...
Call KNTN_UpdateRecords()

' -----------------------------------------------------------------------
' Sub / Function
' -----------------------------------------------------------------------
Sub KNTN_UpdateRecords()
  Dim kntn_api_uri
  Dim responseText
  Dim sendData
  Dim kntn_app_id
  ...
  call KNTN_getHeaderArray(json_properties,"レコード更新用フィールド取得",nameOrCode,RootArray_UodateApiFields,excelSheetName,lookUpArray,False)
  array_header = RootArray_UodateApiFields.item(excelSheetName)
