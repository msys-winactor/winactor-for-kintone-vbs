...
Call KNTN_AddRecordsByCsv()

' -----------------------------------------------------------------------
' Sub / Function
' -----------------------------------------------------------------------
Sub KNTN_AddRecordsByCsv()
  Dim kntn_api_uri
  Dim responseText
  Dim sendData
  Dim kntn_app_id
  ...
  set json_fieldsInfo = json_fieldsInfo.properties

  dim array_fieldsinfo,RootArray_AddApiFields,outputType_rootArray,lookUpArray,array_header
	set RootArray_AddApiFields = WScript.CreateObject("Scripting.Dictionary")

  'Lookupのmappingの項目の一覧を取得する
  lookUpArray = KNTN_CreateLookUpArray(json_fieldsInfo)

  'APIで利用するヘッダー情報をまとめたArrayを求める
  call Kntn_getHeaderArray(json_fieldsInfo,"レコード登録用フィールド取得（ヘッダーのみ）",nameOrCode,RootArray_AddApiFields,"main",lookUpArray,False)
  array_header = RootArray_AddApiFields.item("main")
  ...
  call KNTN_checkDuplicateFields(Array_EntryData,1)
	...
	Array_OutputData(0,resultCol+1) = "レコードID"
	Array_OutputData(0,resultCol+2) = "処理実行日時"
	Array_OutputData(0,resultCol+3) = "エラー内容"
  ...
  dim aryStrings
    ...
    if errMsg <> "" then
      ...
      Array_OutputData(i-1,resultCol+2) = KNTN_GetNow
      Array_OutputData(i-1,resultCol+3) = errmsg
      
      'エラー配列に既存データを格納
      for j = 1 to ubound(Array_EntryData,2)
        Array_ErrorData(errorRecordCount,j) = Array_EntryData(i,j) 
      next
    else 
      sendData = "{app:" &  kntn_app_id & ",record:{" & recordInfo  & "}}" 
      ...
      With wscript.CreateObject("MSXML2.XMLHTTP")
        ...
        Case 200
          ...
          Array_OutputData(i-1,resultCol+1) = result_json.id
          Array_OutputData(i-1,resultCol+2) = KNTN_GetNow
          Array_OutputData(i-1,resultCol+3) = ""

        Case Else
          '処理結果配列に格納する
          errorRecordCount = errorRecordCount +1
          errmsg = "Kintoneのレコード登録操作に失敗しました。" & vbCrLf & _
