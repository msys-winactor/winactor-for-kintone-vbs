...
' Kintoneのレコードを取得
Call Kntn_GetCustomFieldsRecordsToCsv()

' -----------------------------------------------------------------------
' Sub / Function
' -----------------------------------------------------------------------
Sub Kntn_GetCustomFieldsRecordsToCsv()
    Dim kntn_api_uri
    Dim responseText
    Dim sendData
    Dim kntn_app_id
    ...
    If objFSO.FileExists(objFSO.BuildPath(saveFolderPath, csvFileName & ".csv")) = True And canOverWriteFile = "エラー" Then
        Err.Raise 1, "", "保存先にcsvファイルが既に存在しています。既存ファイルを移動する、または保存先を変更してください。"
    End If
    csvFilePath = objFSO.BuildPath(saveFolderPath, csvFileName & ".csv")
    ...
    For i = 0 To UBound(array_fields)
        ...
        For j = 0 To keysLength - 1
            'JScript9対応: getKeysで取得したキー配列からインデックスで取得
            Dim key
