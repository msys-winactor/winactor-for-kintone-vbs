...
Function Kntn_CheckAccessToken(kntnClientId)
  ...
  tokenInfo = Kntn_DecodeCryptText(crypt_access_token, kntn_crypt_keyword) & "," & kntn_expires_date & "," & Kntn_DecodeCryptText(crypt_refresh_token, kntn_crypt_keyword)
...
Function Kntn_CheckAccessTokenValidity(accessToken, expiresDate, refreshToken)
  ...
  If DateDiff("n", Now, expiresDate) < 10 Then
    ...
    If Kntn_RenewAccessToken(refresh_token) Then
  ...
  With wscript.CreateObject("MSXML2.XMLHTTP")
    ...
    select case .status
      case 200
        ...
        objHtml.write "<script>document.JsonParse=function (s) {return eval('(' + s + ')');}</script>"

        ' JSONをパースする
        Dim getData
        Set getData = objHtml.JsonParse(responseText)
